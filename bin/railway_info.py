...
def fetch_line_color(info_json):
    ...
    for comp in resolve_list:
        if info_json is not None:
            lines = next((x['lines'] for x in info_json if x['comp_name'] == comp['comp_name'] and x['comp_type']['code'] == comp['comp_code']))
        ...
        for rule in comp['rules']:
            ...
            for col in TableResolver(table):
                ...
                line = next((x for x in lines if x['line_name'] == key), None) if info_json is not None else None
                if line is not None:
                    line['line_color'] = val
                else:
                    lines_not_found.append({'line_name': key, 'line_color': val})
        
        line_color_list.append({'comp_name': comp['comp_name'], 'comp_code': comp['comp_code'], 'lines': lines_not_found})
...
def gen_info(geojson):
    ...
    for cname, ctype in comp_list:
        ...
        comp_line_list.append({'comp_name': cname, 'comp_type': { 'name': ctype_dict[ctype], 'code': ctype }, 'lines': line_list})

    sorted_list = []
    for item in comp_line_list:
        item['lines'] = sorted(item['lines'], key=lambda x: x['line_type']['code'])
        sorted_list.append(item)

    return sorted(sorted_list, key=lambda x: x['comp_type']['code'])


if __name__ == '__main__':
    parser = ArgumentParser(description='generate railway info from geojson and wikipedia.')
    parser.add_argument('--input', '-i', help='input file.')
    parser.add_argument('--output-info-json', help='path of generated file.')
    parser.add_argument('--output-line-color', help='path of line color file if some line color not fit to infojson.')
    parser.add_argument('--line-color-only', '-c', action='store_true',help='only generate line color info.')
    args = parser.parse_args()

    if args.line_color_only:
        if args.input is not None:
            info = json.loads(Path(args.input).read_text())
            ...
            line_color_left = fetch_line_color(None)
            if len(line_color_left) > 0:
                Path(args.output_line_color).write_text(json.dumps(line_color_left, ensure_ascii=False, indent=2), encoding='utf-8')
